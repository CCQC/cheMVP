...
DrawingInfo::DrawingInfo()
    : myXRot(0), myYRot(0), myZRot(0), _useFogging(false), _foggingScale(DEFAULT_FOGGING_SCALE),
      _usePerspective(true), _perspectiveScale(DEFAULT_PERSPECTIVE_SCALE),
      myDX((int)(DEFAULT_SCENE_SIZE_X / 2.0)), myDY((int)(DEFAULT_SCENE_SIZE_Y / 2.0)), myUserDX(0),
      myUserDY(0), myMidX((int)(DEFAULT_SCENE_SIZE_X / 2.0)),
      myMidY((int)(DEFAULT_SCENE_SIZE_Y / 2.0)), myWidth((int)(DEFAULT_SCENE_SIZE_X)),
      myHeight((int)(DEFAULT_SCENE_SIZE_Y)), myUserScaleFactor(100.0), myMoleculeMaxDimension(1.0),
      myAngToSceneScale(1), _maxZ(0.0), _minZ(0.0), _maxBondZ(0.0), _minBondZ(0.0),
      _anglePenWidth(0.2), _angleColor(Qt::black), _anglePen(Qt::black),
      _anglePrecision(DEFAULT_ANGLE_LABEL_PRECISION), _bondColor(Qt::black),
      _bondPrecision(DEFAULT_BOND_LABEL_PRECISION), _labelColor(Qt::black),
      _atomLabelFont(DEFAULT_ATOM_LABEL_FONT), _atomLineColor(Qt::black), _atomTextColor(Qt::black),
      style(SimpleColored)
{
}
...
{
    // To make the the user scale factor run from 0 to 100%, divide by 100, and we need to double
    // the radius
    // of the molecule to find the circumference - the 200 is not a random number!
    myAngToSceneScale =
        (myWidth > myHeight ? myHeight : myWidth) / (200.0 * myMoleculeMaxDimension);
...
void DrawingInfo::serialize(QXmlStreamWriter *writer)
{
    ...
    writer->writeAttribute("angleColor",
                           QString("%1 %2 %3 %4")
                               .arg(_angleColor.red())
                               .arg(_angleColor.green())
                               .arg(_angleColor.blue())
                               .arg(_angleColor.alpha()));
    writer->writeAttribute("anglePrecision", QString("%1").arg(_anglePrecision));
    writer->writeAttribute("bondColor",
                           QString("%1 %2 %3 %4")
                               .arg(_bondColor.red())
                               .arg(_bondColor.green())
                               .arg(_bondColor.blue())
                               .arg(_bondColor.alpha()));
    writer->writeAttribute("bondPrecision", QString("%1").arg(_bondPrecision));
    writer->writeAttribute("labelColor",
                           QString("%1 %2 %3 %4")
                               .arg(_labelColor.red())
                               .arg(_labelColor.green())
                               .arg(_labelColor.blue())
                               .arg(_labelColor.alpha()));
    writer->writeAttribute("atomLColor",
                           QString("%1 %2 %3 %4")
                               .arg(_atomLineColor.red())
                               .arg(_atomLineColor.green())
                               .arg(_atomLineColor.blue())
                               .arg(_atomLineColor.alpha()));
    writer->writeAttribute("atomTColor",
                           QString("%1 %2 %3 %4")
                               .arg(_atomTextColor.red())
                               .arg(_atomTextColor.green())
                               .arg(_atomTextColor.blue())
                               .arg(_atomTextColor.alpha()));
    writer->writeAttribute("style", QString("%1").arg(style));
    writer->writeAttribute(
        "atomFont", QString("%1 %2").arg(_atomLabelFont.family()).arg(_atomLabelFont.pointSize()));
...
DrawingInfo *DrawingInfo::deserialize(QXmlStreamReader *reader)
{
    ...
    d->_bondColor = QColor(
        bondColor[0].toInt(), bondColor[1].toInt(), bondColor[2].toInt(), bondColor[3].toInt());
    d->_bondPrecision = attr.value("bondPrecision").toString().toInt();
    QStringList labelColor = attr.value("labelColor").toString().split(" ");
    d->_labelColor = QColor(
        labelColor[0].toInt(), labelColor[1].toInt(), labelColor[2].toInt(), labelColor[3].toInt());
    QStringList atomLColor = attr.value("atomLColor").toString().split(" ");
    d->_atomLineColor = QColor(
        atomLColor[0].toInt(), atomLColor[1].toInt(), atomLColor[2].toInt(), atomLColor[3].toInt());
    QStringList atomTColor = attr.value("atomTColor").toString().split(" ");
    d->_atomTextColor = QColor(
        atomTColor[0].toInt(), atomTColor[1].toInt(), atomTColor[2].toInt(), atomTColor[3].toInt());
