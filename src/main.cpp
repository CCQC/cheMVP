...
// Set the icon in the bar at the top of the window but not on X11 - it seems to
// be having problems
#ifndef Q_WS_X11
    ...
    if (argv <= 1) {
        ...
        if (cmd_line_arg == "--help" || cmd_line_arg == "-h" || argv > 3) {
            std::cout << "Usage: chemvp [coordfile]" << std::endl
                      << "Where coordfile is an xyz file" << std::endl;
            exit(EXIT_FAILURE);
        } else {
        }
    }
