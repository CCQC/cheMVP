...
void MainWindow::createActions()
{
    ...
    insertAngstromAction->setToolTip(tr("Insert ") + QChar((ushort)0x00C5) +
                                     tr(" at current cursor position"));
    ...
    insertDegreeAction->setToolTip(tr("Insert ") + QChar((ushort)0x00B0) +
                                   tr(" at current cursor position"));
    ...
    insertPlusMinusAction->setToolTip(tr("Insert ") + QChar((ushort)0x00B1) +
                                      tr(" at current cursor position"));

    connect(insertTextActionGroup,
            SIGNAL(triggered(QAction *)),
            this,
            SLOT(insertTextAtCursor(QAction *)));
}
...
void MainWindow::deleteItem()
{
    ...
    // Prevents bond deletion from being undoable, but fixes geometry time step from reinstating
    // deleted bond
