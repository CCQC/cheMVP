...
void MainWindow::createToolbars()
{
    ...
    connect(mouseModeButtonGroup,
            SIGNAL(buttonClicked(int)),
            this,
            SLOT(mouseModeButtonGroupClicked(int)));
    ...
    rotateButton->setToolTip(tr("Rotate Molecule\nClick and drag to rotate about x and y\nHold "
                                "shift, click and drag to rotate about z"));
    ...
    addBondButton->setToolTip(tr("Add Bond\nClick an atom, (keep the button held down) and release "
                                 "on another atom to form a new bond"));
    ...
    textFontSizeCombo->setCurrentIndex(
        textFontSizeCombo->findText(QString().setNum(DEFAULT_LABEL_FONT_SIZE)));
...
void MainWindow::disableLabelSignals()
{
    ...
    disconnect(
        textFontCombo, SIGNAL(currentFontChanged(const QFont &)), this, SLOT(setLabelFont(QFont)));
    disconnect(textFontSizeCombo,
               SIGNAL(currentIndexChanged(const QString &)),
               this,
               SLOT(setLabelFontSize(QString)));
}
...
void MainWindow::updateTextLabelToolbar()
{
    ...
    // called every time the selected text changes, but the atoms, bonds, etc. might not have
    // changed
        ...
        foreach (QGraphicsItem *item, canvas->items()) {
            if (ITEM_IS_LABEL) {
                ...
                if (label->isSelected() ||
                    ...
                    textFontSizeCombo->setCurrentIndex(
                        textFontSizeCombo->findText(QString().setNum(currentFont.pointSize())));
