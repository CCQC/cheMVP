...
class Arrow : public QGraphicsLineItem
{
  public:
    ...
    {
        myStartBox->setDX(myStartBox->dX() + val);
        myEndBox->setDX(myEndBox->dX() + val);
    }
    void incDY(double val)
    {
        myStartBox->setDY(myStartBox->dY() + val);
        myEndBox->setDY(myEndBox->dY() + val);
    }
    void setThickness(const double val)
    {
        myThickness = val;
        effectiveWidth = drawingInfo->scaleFactor() * val;
    }
    void updatePosition(double x, double y);
    void updatePosition();
    DragBox *startBox() const
    {
        return myStartBox;
    }
    DragBox *endBox() const
    {
        return myEndBox;
    }
    void setAcceptsHovers(bool arg)
